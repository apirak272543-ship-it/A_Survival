...
describe("creator texture router", () => {
  ...
  it("previews the real 300-entry plant catalog against content catalog without runtime import", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.plantContentCatalogPreview({ seed: "plant-catalog-seed", samplePlantCount: 16, samplePerCategory: 8 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ generatorId: "plant.catalog", generatorVersion: "1.0.0", seed: "plant-catalog-seed", plantCount: 300, sampledPlantCount: 16 });
    expect(result.summary.plantCount).toBe(300);
    expect(result.summary.sampledPlantCount).toBe(16);
    expect(result.summary.biomeIds).toEqual(["obsidian-frontier"]);
    expect(result.summary.unresolvedReferenceTypes["seed-definition"]).toBe(16);
    expect(result.summary.unresolvedReferenceTypes["harvest-definition"]).toBe(16);
    expect(result.summary.unresolvedReferenceTypes["soil-context"]).toBe(16);
    expect(result.graph.valid).toBe(false);
    expect(result.graph.issues.some(issue => issue.code === "MISSING_REQUIRED_DEPENDENCY")).toBe(true);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  });

  it("previews world spawn loot through item.universal without runtime import", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.worldSpawnLootUniversalItemPreview({ seed: "loot-universal-router-seed", radius: 32, sampleSpawnCount: 64, maxPowerBudget: 100 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ mapId: "obsidian-frontier", seed: "loot-universal-router-seed", lootGeneratorVersion: "0.1.0", universalItemGeneratorVersion: "1.0.0" });
    expect(result.summary.sampledSpawnCount).toBe(14);
    expect(result.summary.lootCount).toBeGreaterThan(0);
    expect(result.summary.dropCount).toBeGreaterThan(0);
    expect(result.summary.universalItemCount + result.summary.blockedItemCount).toBe(result.summary.dropCount);
    expect(result.summary.assetIds.length).toBeGreaterThan(0);
    expect(result.summary.unresolvedReferenceCount).toBeGreaterThan(0);
    expect(result.graph.valid).toBe(false);
    expect(result.graph.issues.some(issue => issue.code === "MISSING_REQUIRED_DEPENDENCY")).toBe(true);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  ...
  it("previews story progression runtime state against quest progression without runtime import", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.storyProgressionContractPreview({ seed: "story-runtime-router-seed", completedQuestCount: 20 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ mapId: "obsidian-frontier", seed: "story-runtime-router-seed", questGeneratorVersion: "1.0.0", runtimeStoryVersion: "1.0.0", completedQuestCount: 20, questCount: 40 });
    expect(result.runtimeSummary).toMatchObject({ playableMapId: "obsidian-frontier", completedQuestCount: 20, questsPerPlayableMap: 20, currentQuestId: null, completedMapIndex: 1, nextMapReadyIndex: 2, futureMapsRuntimeImportAllowed: false });
    expect(result.summary).toMatchObject({ currentQuestMatch: true, completedQuestPrefixMatch: true, playableMapContractMatch: true, nextMapReadyIndex: 2, nextMapRuntimeImportAllowed: false, futureMapsRuntimeImportAllowed: false, unresolvedReferenceCount: 0 });
    expect(result.generated.nextMap).toMatchObject({ mapId: "story-map-002", runtimeStatus: "planned", runtimeImportAllowed: false });
    expect(result.graph.valid).toBe(true);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  });

  it("previews story map selection and cache policy without preparing future maps", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.storyMapCachePolicyPreview({ seed: "story-map-cache-router-seed", requestedMapIds: ["obsidian-frontier", "story-map-002"], completedQuestCount: 20 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ runtimeMapId: "obsidian-frontier", seed: "story-map-cache-router-seed", requestedMapIds: ["obsidian-frontier", "story-map-002"] });
    expect(result.summary).toMatchObject({ runtimeMapId: "obsidian-frontier", storyPlayableMapId: "obsidian-frontier", storyContractConsistent: true, requestedCount: 2, selectionAllowedCount: 1, cacheEligibleCount: 1, fallbackCount: 1, registryDefinitionMissingCount: 1, runtimeDeniedCount: 1, runtimeImportAllowed: false, playerVisible: false, cacheable: false });
    expect(result.requests[1]).toMatchObject({ requestedMapId: "story-map-002", resolvedMapId: "obsidian-frontier", selectionAllowed: false, cacheEligible: false, fallsBackToRuntimeMap: true, registryDefinitionAvailable: false });
    expect(result.graph.valid).toBe(false);
    expect(result.graph.issues.some(issue => issue.code === "MISSING_REQUIRED_DEPENDENCY")).toBe(true);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  });

  it("previews quest gameplay event bridge without completing quests or importing runtime", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.questGameplayEventPreview({ seed: "quest-event-router-seed", sampleQuestCount: 8, completedQuestCount: 20 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ mapId: "obsidian-frontier", seed: "quest-event-router-seed", sampleQuestCount: 8, completedQuestCount: 20 });
    expect(result.summary).toMatchObject({ sampleQuestCount: 8, supportedObjectiveCount: 0, unsupportedObjectiveCount: 8, missingRuntimeEventCount: 5, missingTargetBindingCount: 8, unresolvedReferenceCount: 13, runtimeImportAllowed: false, playerVisible: false, cacheable: false });
    expect(result.assessments[0]).toMatchObject({ objectiveKind: "visit", supported: false });
    expect(result.graph.valid).toBe(false);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  });

  it("previews quest reward item and ability runtime bridge without applying rewards", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.questRewardRuntimePreview({ seed: "reward-router-seed", sampleQuestCount: 8, completedQuestCount: 20 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ mapId: "obsidian-frontier", seed: "reward-router-seed", sampleQuestCount: 8, completedQuestCount: 20 });
    expect(result.summary).toMatchObject({ rewardCount: 8, itemDefinitionAvailableCount: 8, rewardInstanceFactoryAvailableCount: 8, questRewardDispatchBridgeMissingCount: 8, abilityRewardCount: 0, abilityRuntimeOwnerMissingCount: 0, supportedRewardCount: 0, unsupportedRewardCount: 8, unresolvedReferenceCount: 8, runtimeImportAllowed: false, playerVisible: false, cacheable: false });
    expect(result.assessments[0]).toMatchObject({ questId: "story-map-001-quest-01", itemDefinitionId: "material-001", itemDefinitionAvailable: true, rewardInstanceFactoryAvailable: true, supported: false });
    expect(result.graph.valid).toBe(false);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  });
  ...
  it("previews quest reward dispatch gates without persisting or emitting gameplay", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.questRewardDispatchPreview({ seed: "reward-dispatch-router", completedQuestCount: 0, sequenceBase: 10 });
  ...
  it("previews story offline map state namespace without writing IndexedDB", async () => {
    ...
    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ runtimeMapId: "obsidian-frontier", seed: "story-offline-state-router-seed", playerId: "preview-player", requestedMapIds: ["obsidian-frontier", "story-map-002"] });
    expect(result.assessments).toMatchObject([
      { requestedMapId: "obsidian-frontier", resolvedMapId: "obsidian-frontier", offlineStateNamespaceAllowed: true, futureMapWriteBlocked: false },
      { requestedMapId: "story-map-002", resolvedMapId: "obsidian-frontier", offlineStateNamespaceAllowed: false, futureMapWriteBlocked: true },
    ]);
    expect(result.summary).toMatchObject({ requestedCount: 2, selectableCount: 1, cacheEligibleCount: 1, offlineStateNamespaceAllowedCount: 1, futureMapWriteBlockedCount: 1, runtimeDeniedCount: 1, runtimeImportAllowed: false, playerVisible: false, cacheable: false });
    expect(result.graph.valid).toBe(false);
    expect(result.graph.issues.some(issue => issue.code === "MISSING_REQUIRED_DEPENDENCY" && issue.dependencyKey === "offline-state-runtime-approval:story-map-002")).toBe(true);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  });

  it("previews MAP_001 encounter story gate against quest progression without runtime import", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.map001StoryGatePreview({ seed: "map001-story-gate-router-seed", mapCount: 2 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ mapId: "obsidian-frontier", seed: "map001-story-gate-router-seed", mapCount: 2, questCount: 40, playableMapIndex: 1, nextMapIndex: 2, nextMapRuntimeImportAllowed: false, questGeneratorVersion: "1.0.0", encounterOwnerVersion: "1.0.0" });
    expect(result.summary).toMatchObject({ playableMapQuestCount: 20, nextMapId: "story-map-002", nextMapPrerequisiteCount: 20, encounterEventCount: 2, encounterCompletionSupported: false, gateReady: false, futureMapRuntimeImportAllowed: false });
    expect(result.gate.requiredQuestIds).toHaveLength(20);
  ...
  it("previews MAP_001 encounter events against world spawn and loot context without runtime import", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.map001EncounterPreview({ seed: "map001-encounter-router-seed", radius: 32, sampleSpawnCount: 64 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ mapId: "obsidian-frontier", seed: "map001-encounter-router-seed", worldGeneratorVersion: "0.1.0", encounterOwnerVersion: "1.0.0", lootGeneratorVersion: "0.1.0", triggerCount: 2 });
    expect(result.summary.eventKinds).toEqual(["distress-pod", "void-reaper"]);
    expect(result.summary.expectedSpeciesIds).toEqual(["glass-stalker", "void-reaper"]);
  ...
  it("previews procedural weapons through item.universal without runtime import", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.proceduralUniversalItemPreview({ seed: 829173, count: 8, category: "melee", maxPowerBudget: 100 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ generatorId: "content.generator", generatorVersion: "0.1.0", seed: 829173, generatedWeaponCount: 8, universalItemCount: 8, blockedItemCount: 0 });
    expect(result.summary.generatedWeaponCount).toBe(8);
    expect(result.summary.universalItemCount).toBe(8);
    expect(result.summary.blockedItemCount).toBe(0);
    expect(result.summary.assetIds).toEqual(["items.blade"]);
    expect(result.summary.balanceScores).toHaveLength(8);
  ...
  it("previews real structure spawn intent against world spawn context without runtime import", async () => {
    const caller = appRouter.createCaller(createContext("admin"));
    const result = await caller.creator.dependencyGraph.structureSpawnReconciliationPreview({ seed: "structure-spawn-router-seed", radius: 32, blueprintIds: ["compound-frontier-farm"], sampleSpawnCount: 64 });

    expect(result.previewOnly).toBe(true);
    expect(result.artifact).toMatchObject({ mapId: "obsidian-frontier", seed: "structure-spawn-router-seed", worldGeneratorVersion: "0.1.0", structureGeneratorVersion: "1.0.0", structurePlacementCount: 1 });
    expect(result.summary.structurePlacementCount).toBe(1);
    expect(result.summary.structureIntentCount).toBeGreaterThan(0);
    expect(result.summary.expectedSpeciesIds).toContain("field-tender");
    expect(result.summary.unresolvedReferenceCount).toBeGreaterThan(0);
    expect(result.graph.valid).toBe(false);
    expect(result.graph.issues.some(issue => issue.code === "MISSING_REQUIRED_DEPENDENCY")).toBe(true);
    expect(result.graph.runtimePolicy).toEqual({ runtimeImportAllowed: false, playerVisible: false, cacheable: false });
  ...
  it("keeps generator writes out of regular player users", async () => {
    ...
    await expect(caller.creator.dependencyGraph.structureSpawnReconciliationPreview({ seed: "blocked-seed", radius: 32, blueprintIds: ["compound-frontier-farm"], sampleSpawnCount: 16 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.creator.dependencyGraph.worldSpawnLootUniversalItemPreview({ seed: "blocked-seed", radius: 32, sampleSpawnCount: 16, maxPowerBudget: 100 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.creator.dependencyGraph.map001EncounterPreview({ seed: "blocked-seed", radius: 32, sampleSpawnCount: 16 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.creator.dependencyGraph.map001StoryGatePreview({ seed: "blocked-seed", mapCount: 2 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.creator.dependencyGraph.storyProgressionContractPreview({ seed: "blocked-seed", completedQuestCount: 0 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.creator.dependencyGraph.storyMapCachePolicyPreview({ seed: "blocked-seed", requestedMapIds: ["obsidian-frontier"] })).rejects.toMatchObject({ code: "FORBIDDEN" });
    ...
    await expect(caller.creator.dependencyGraph.questRewardDispatchPreview({ seed: "blocked-seed", completedQuestCount: 0 })).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
