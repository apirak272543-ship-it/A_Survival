...
export default function CreatorDomainWorkbench() {
  ...
  const [compositionPixels, setCompositionPixels] = useState<Record<string, string>>({});
  ...
  const busy = worldPreview.isPending || blockPreview.isPending || structurePreview.isPending || itemPreview.isPending || weaponPreview.isPending || animationPreview.isPending || compositionPreview.isPending || compositionTexturePreview.isPending || questPreview.isPending || profilerPreview.isPending || artifactPreview.isPending || artifactRegister.isPending || artifactReview.isPending || artifactExport.isPending || artifactCompatibility.isPending;
  const lastError = worldPreview.error ?? blockPreview.error ?? structurePreview.error ?? itemPreview.error ?? weaponPreview.error ?? animationPreview.error ?? compositionPreview.error ?? compositionTexturePreview.error ?? questPreview.error ?? profilerPreview.error ?? artifactPreview.error ?? artifactRegister.error ?? artifactReview.error ?? artifactExport.error ?? artifactCompatibility.error ?? artifactList.error ?? artifactAudit.error;
  ...
  const toggleCompositionPixel = (x: number, y: number) => setCompositionPixels(previous => { const key = `${x}:${y}`; if (previous[key] === compositionSelectedColor) { const next = { ...previous }; delete next[key]; return next; } return { ...previous, [key]: compositionSelectedColor }; });
  const buildCompositionRequest = () => ({ templateId: compositionTemplateId, subject: compositionSubject as "block" | "structure" | "item" | "weapon" | "animation", canvasWidth: Number(compositionWidth), canvasHeight: Number(compositionHeight), layers: [{ id: "base", label: "พื้นฐาน", role: "base" as const, zIndex: 0, visible: true, opacity: 1 }, { id: "outline", label: "เส้นขอบ", role: "outline" as const, zIndex: 10, visible: true, opacity: 0.9 }], parts: [{ id: "body", label: "ส่วนหลัก", slot: "body" as const, x: Number(compositionPartX), y: Number(compositionPartY), width: Number(compositionPartWidth), height: Number(compositionPartHeight), layerIds: ["base", "outline"] }], palette: compositionPalette, pixels: Object.entries(compositionPixels).map(([key, colorId]) => { const [x, y] = key.split(":").map(Number); return { x, y, colorId }; }) });
