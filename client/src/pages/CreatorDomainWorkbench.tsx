...
export default function CreatorDomainWorkbench() {
  ...
  const busy = worldPreview.isPending || blockPreview.isPending || structurePreview.isPending || itemPreview.isPending || weaponPreview.isPending || animationPreview.isPending || compositionPreview.isPending || questPreview.isPending || profilerPreview.isPending || artifactPreview.isPending || artifactRegister.isPending || artifactReview.isPending || artifactExport.isPending || artifactCompatibility.isPending;
  const lastError = worldPreview.error ?? blockPreview.error ?? structurePreview.error ?? itemPreview.error ?? weaponPreview.error ?? animationPreview.error ?? compositionPreview.error ?? questPreview.error ?? profilerPreview.error ?? artifactPreview.error ?? artifactRegister.error ?? artifactReview.error ?? artifactExport.error ?? artifactCompatibility.error ?? artifactList.error ?? artifactAudit.error;
  ...
  const runPreview = () => {
    ...
    if (domain === "composition") {
      compositionPreview.mutate({ templateId: compositionTemplateId, subject: compositionSubject as "block" | "structure" | "item" | "weapon" | "animation", canvasWidth: Number(compositionWidth), canvasHeight: Number(compositionHeight), layers: [{ id: "base", label: "พื้นฐาน", role: "base", zIndex: 0, visible: true, opacity: 1 }, { id: "outline", label: "เส้นขอบ", role: "outline", zIndex: 10, visible: true, opacity: 0.9 }], parts: [{ id: "body", label: "ส่วนหลัก", slot: "body", x: Number(compositionPartX), y: Number(compositionPartY), width: Number(compositionPartWidth), height: Number(compositionPartHeight), layerIds: ["base", "outline"] }], palette: compositionPalette, pixels: Object.entries(compositionPixels).map(([key, colorId]) => { const [x, y] = key.split(":").map(Number); return { x, y, colorId }; }) }, { onSuccess: result => setStatus(`composition preview สำเร็จ · ${result.summary.paintedPixelCount} pixel · ${result.registryMetadata.contentSha256.slice(0, 12)}…`) });
      return;
    }
    if (domain === "quest") {
