...
import { CREATOR_TEMPLATE_PRESETS, CREATOR_WORKBENCH_COMPOSITION_PARTS, getCompositionSubjectForTemplate, isWorkbenchCompositionTemplate } from "@/lib/creatorTemplateCatalog";
...
export default function CreatorDomainWorkbench() {
  ...
  const [compositionSelectedPartIds, setCompositionSelectedPartIds] = useState<string[]>(["torso"]);
  const [compositionTextureArtifactKey, setCompositionTextureArtifactKey] = useState("");
  const [compositionTextureReviewNote, setCompositionTextureReviewNote] = useState("");
  ...
  const compositionTextureReview = trpc.creator.texture.review.useMutation();
  const compositionTextureArtifactList = trpc.creator.texture.list.useQuery({ limit: 20 }, { enabled: domain === "composition" });
  const compositionTextureArtifactAudit = trpc.creator.texture.audit.useQuery({ artifactKey: compositionTextureArtifactKey || "pending-texture-artifact-key", limit: 20 }, { enabled: domain === "composition" && Boolean(compositionTextureArtifactKey) });
  ...
  const busy = worldPreview.isPending || blockPreview.isPending || structurePreview.isPending || itemPreview.isPending || weaponPreview.isPending || animationPreview.isPending || compositionPreview.isPending || compositionTexturePreview.isPending || compositionTextureExportPreview.isPending || compositionTextureByteCompatibility.isPending || compositionTextureRegister.isPending || compositionTextureReview.isPending || questPreview.isPending || profilerPreview.isPending || artifactPreview.isPending || artifactRegister.isPending || artifactReview.isPending || artifactExport.isPending || artifactCompatibility.isPending;
  const lastError = worldPreview.error ?? blockPreview.error ?? structurePreview.error ?? itemPreview.error ?? weaponPreview.error ?? animationPreview.error ?? compositionPreview.error ?? compositionTexturePreview.error ?? compositionTextureExportPreview.error ?? compositionTextureByteCompatibility.error ?? compositionTextureRegister.error ?? compositionTextureReview.error ?? questPreview.error ?? profilerPreview.error ?? artifactPreview.error ?? artifactRegister.error ?? artifactReview.error ?? artifactExport.error ?? artifactCompatibility.error ?? artifactList.error ?? artifactAudit.error ?? compositionTextureArtifactList.error ?? compositionTextureArtifactAudit.error;
  ...
  const compositionPartPresets = Object.values(CREATOR_WORKBENCH_COMPOSITION_PARTS);
  ...
  const toggleCompositionPart = (partId: string) => setCompositionSelectedPartIds(previous => { if (previous.includes(partId)) return previous.length === 1 ? previous : previous.filter(id => id !== partId); return [...previous, partId]; });
  const buildCompositionParts = () => compositionSubject === "animation" ? compositionPartPresets.filter(part => compositionSelectedPartIds.includes(part.id)).map(part => ({ id: part.id, label: part.label, slot: part.slot, x: part.x, y: part.y, width: part.width, height: part.height, layerIds: compositionLayers.map(layer => layer.id) })) : [{ id: "body", label: "ส่วนหลัก", slot: "body" as const, x: Number(compositionPartX), y: Number(compositionPartY), width: Number(compositionPartWidth), height: Number(compositionPartHeight), layerIds: compositionLayers.map(layer => layer.id) }];
  const buildCompositionRequest = () => ({ templateId: compositionTemplate.id, subject: compositionSubject as "block" | "structure" | "item" | "weapon" | "animation", canvasWidth: Number(compositionWidth), canvasHeight: Number(compositionHeight), layers: compositionLayers, parts: buildCompositionParts(), palette: compositionPalette, pixels: Object.entries(compositionPixels).map(([key, cell]) => { const [, xText, yText] = key.split(":"); return { x: Number(xText), y: Number(yText), ...cell }; }) });
  ...
  const runCompositionTextureRegister = () => compositionTextureRegister.mutate({ ...buildCompositionRequest(), source: compositionSource, provenanceRef: compositionProvenanceRef, textureSampling: compositionTextureSampling }, { onSuccess: result => { setCompositionTextureArtifactKey(result.artifact.artifactKey); setStatus(`ลงทะเบียน texture pack สำเร็จ · ${result.artifact.artifactKey} · runtime import ปิด`); void compositionTextureArtifactList.refetch(); }, onError: error => setStatus(`ลงทะเบียนไม่สำเร็จ · ${error.message} · ไม่ได้ publish เข้า player`) });
  const runCompositionTextureReview = (action: "approve" | "reject" | "reopen") => { if (!compositionTextureArtifactKey) { setStatus("ต้องลงทะเบียน texture pack ก่อนจึง review ได้"); return; } compositionTextureReview.mutate({ artifactKey: compositionTextureArtifactKey, action, note: compositionTextureReviewNote }, { onSuccess: result => { setStatus(`texture review เปลี่ยนเป็น ${result.artifact.reviewStatus} · runtime import ปิด`); void compositionTextureArtifactList.refetch(); void compositionTextureArtifactAudit.refetch(); } }); };
  ...
  const renderResult = () => {
    ...
    if (domain === "composition" && (compositionTextureRegister.data || compositionTextureReview.data)) {
      const artifact = compositionTextureReview.data?.artifact ?? compositionTextureRegister.data?.artifact;
      const compatibility = compositionTextureRegister.data?.compatibility;
      if (!artifact) return null;
      const reviewStatus = artifact.reviewStatus;
      return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ResultPill label="artifact key" value={`${artifact.artifactKey.slice(0, 22)}…`} /><ResultPill label="review" value={reviewStatus} /><ResultPill label="compatibility" value={compatibility?.decision ?? "ตรวจแล้วก่อน register"} /><ResultPill label="runtime import" value="ปิด" /></div><div className="flex flex-wrap items-end gap-3 rounded-xl border border-emerald-300/15 bg-emerald-300/[0.04] p-3"><Field label="หมายเหตุ review texture" htmlFor="composition-texture-review-note"><Input id="composition-texture-review-note" value={compositionTextureReviewNote} onChange={event => setCompositionTextureReviewNote(event.target.value)} placeholder="จำเป็นเมื่อปฏิเสธหรือเปิดกลับมาตรวจใหม่" className="min-w-64 border-white/10 bg-white/[0.04]" /></Field><div className="flex flex-wrap gap-2">{reviewStatus === "draft" && <><Button onClick={() => runCompositionTextureReview("approve")} disabled={busy} variant="outline" className="border-emerald-300/30 text-emerald-200">อนุมัติ texture</Button><Button onClick={() => runCompositionTextureReview("reject")} disabled={busy} variant="outline" className="border-rose-300/30 text-rose-200">ปฏิเสธ texture</Button></>}{reviewStatus === "rejected" && <Button onClick={() => runCompositionTextureReview("reopen")} disabled={busy} variant="outline" className="border-amber-300/30 text-amber-200">เปิดกลับมาตรวจ</Button>}</div><span className="text-[11px] leading-relaxed text-slate-500">review เป็นขั้นตอนแยก · ยังไม่ publish/import/cache เข้า player · ต้องมี DB และ object storage</span></div>{compositionTextureArtifactList.data && <div className="space-y-2"><p className="text-xs font-bold text-slate-200">texture artifacts ล่าสุด ({compositionTextureArtifactList.data.length})</p>{compositionTextureArtifactList.data.slice(0, 5).map(item => <div key={item.artifactKey} className="rounded-lg border border-white/8 bg-black/15 px-3 py-2"><p className="text-xs text-slate-200">{item.packId}@{item.packVersion} · review {item.reviewStatus}</p><p className="mt-1 truncate text-[10px] text-slate-500">{item.artifactKey}</p></div>)}</div>}{compositionTextureArtifactAudit.data && <div className="space-y-2"><p className="text-xs font-bold text-slate-200">ประวัติ review texture ({compositionTextureArtifactAudit.data.length})</p>{compositionTextureArtifactAudit.data.map(event => <div key={event.id} className="rounded-lg border border-white/8 bg-black/15 px-3 py-2"><p className="text-xs text-slate-200">{event.action} · {event.fromStatus} → {event.toStatus}</p><p className="mt-1 text-[10px] text-slate-500">ผู้ตรวจ #{event.reviewerUserId} · {event.note || "ไม่มีหมายเหตุ"}</p></div>)}</div>}<div className="rounded-xl border border-amber-300/15 bg-amber-300/[0.04] p-3 text-xs leading-relaxed text-amber-100/80">texture pack อยู่ใน developer registry เท่านั้น · review อนุมัติไม่เท่ากับ publish · `runtimeImportAllowed` ยังปิด</div></div>;
    }
