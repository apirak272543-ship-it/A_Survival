...
| `RESERVED` | AI-0 | `NEXT-B02-PHYSICS-CONTRACT-TEST-001` | B-02 sub-checkpoint: exact files `client/src/game/systems/blockPhysicsSystem.ts` and `server/blockPhysicsSystem.test.ts`; add regression coverage for canonical Obsidian slab partial blocking, thin non-solid pass-through, cactus hazard contact and cooldown. PR #22 uses separate `server/blockHazardPlacementCoverageContract.ts` paths and is not a file conflict. No fabricated hazard, future-map expansion, gravity solver or persistence claim |
