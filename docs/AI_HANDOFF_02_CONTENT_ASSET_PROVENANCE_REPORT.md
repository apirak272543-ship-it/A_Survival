...
| `server/generators/creditsSupportersProvenanceDependencyGraph.ts` | เพิ่ม bounded pure audit สำหรับ `ASSET_CREDITS`, explicit supporters registry/consent, duplicate/malformed records และ durable-registry binding โดยไม่แก้ Credits player UI |
| `server/creditsSupportersProvenanceDependencyGraph.test.ts` | เพิ่ม regression tests 5 รายการสำหรับ canonical credits, reference-only blockers, supporter consent, malformed/duplicate records, deterministic hashes และ bounds |
...
| Files reserved | `server/generators/plantAssetProvenanceDependencyGraph.ts`, `server/plantAssetProvenanceDependencyGraph.test.ts`, `server/generators/contentAssetProvenanceDependencyGraph.ts`, `server/contentAssetProvenanceDependencyGraph.test.ts`, `server/generators/assetPackProvenanceDependencyGraph.ts`, `server/assetPackProvenanceDependencyGraph.test.ts`, `server/generators/creditsSupportersProvenanceDependencyGraph.ts`, `server/creditsSupportersProvenanceDependencyGraph.test.ts`, `docs/AI_HANDOFF_02_CONTENT_ASSET_PROVENANCE_REPORT.md` |
...
| Earlier canonical-credit implementation commit | `37cc543` (canonical entry-credit loading) |
| Latest implementation commit | `67d1ce9` (Credits/Supporters provenance audit) |
...
| Check | ผลที่รันจริง |
|---|---|
| Focused provenance suite | ผ่าน `4` test files / `23` tests: plant, generic content, per-entry asset-pack และ Credits/Supporters provenance adapters |
| Focused owner suite | ผ่าน `10` test files / `44` tests: plant/generic/per-entry/Credits-Supporters provenance adapters, plant catalog, plant generator, plant graph, content catalog, content graph และ active asset-pack manifest |
| Full test suite | ผ่าน `108` test files / `429` tests ด้วย `pnpm test -- --run` |
...
Credits/Supporters audit ยืนยันว่า canonical `ASSET_CREDITS` มี 3 records: 1 project-original pack credit และ 2 reference-only records. Reference-only records ถูก block ไม่ถูกนับเป็น distributable credits. ปัจจุบันไม่มี explicit supporters registry (`supporters: null`) จึงเกิด `supporter-registry` blocker; ไม่ตีความการไม่มีข้อมูลเป็น consent หรือ attribution. แม้ credit record ที่ project-original จะตรวจผ่านระดับ record แต่ `verifiedCreditIds` ยังคงว่างจนกว่าจะมี durable registry snapshot จริง

ยังไม่มีการเชื่อม adapter เข้ากับ Creator Workbench, `creatorRouter` หรือ player `CreditsSheet` เพราะเป็น shared/player integration surface ที่ไม่อยู่ใน checkpoint นี้. ยังไม่มี authenticated creator E2E, live database/storage, durable registry write, object-storage upload, asset generation, runtime publish/import/cache acceptance, browser/device/mobile acceptance หรือ final matrix update. Durable registry, direct per-entry credit และ supporters records ถูกออกแบบเป็น injected source contracts เพื่อทดสอบเท่านั้น; active source loader คืนค่า `durableRegistry: null`, สร้าง entry-credit map จาก `getAssetCredit()` ของ canonical owner และคืน `supporters: null` จึงไม่อ้างว่ามี registry, direct credit หรือ supporter consent ถาวรอยู่จริง. Graph runtime policy ยังคงเป็น `{ runtimeImportAllowed: false, playerVisible: false, cacheable: false }`

AI-0 ควรตรวจ diff ของ commit `67d1ce9` (รวมฐานเดิม `37cc543`, `2080ce3`, `5be13ff`, `bea4623` และ `caf2fad`), ตรวจ completion report นี้ และเปลี่ยนสถานะ registry จาก `RESERVED` เมื่อหลักฐานครบตามเกณฑ์ของ AI-0. หากต้องเปิด logical asset blocker ในอนาคต ต้องเพิ่ม file-backed manifest/registry/provenance หลักฐานจริง ไม่ควรแก้ด้วยการเติม metadata หรือเปลี่ยนสถานะเป็น verified
