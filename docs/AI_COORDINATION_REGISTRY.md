...
| `NEXT-CODEX-UI-BOUNDARY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/pages/ArcaneFrontier.tsx` | Base `b78bf9e`; exclusive reservation by AI-0 | เชื่อม `CodexSheet` ที่มีอยู่กับ `createCodexDiscoverySnapshot` เพื่อให้ category counts/list/detail ใช้ discovered-only canonical projection แบบ fail-closed; ไม่เพิ่ม discovery event, session/IndexedDB persistence, network write หรือ player map/control |
| `AI1-O04-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/generatorToolDependencyPolicy.ts`, `server/generatorToolDependencyPolicy.test.ts`, `docs/GENERATOR_TOOL_INVENTORY.md`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ generator implementations, Workbench/router, map/cache/offline/authority | Base `d5e0270b92fd72967ebda12e3e236eb8e4c1652e`; branch `ai1/o04-generator-tool-dependency-policy`; implementation `5db9356` / PR `https://github.com/apirak272543-ship-it/A_Survival/pull/28`; checks `git diff --check`, `pnpm check`, focused 4 files/20 tests, full 117 files/482 tests, heap-limited build ผ่าน | pure inventory/dependency gate ของเครื่องมือ generator ที่ตรวจได้ใน repository; no external execution, secret, background process, live DB migration, player generator UI หรือ fabricated tool success |
