...
| `AI1-M02-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/generators/runtimeVisibilityDependencyGraph.ts`, `server/runtimeVisibilityDependencyGraph.test.ts`, `docs/RUNTIME_VISIBILITY_DEPENDENCY_GRAPH.md`; ไม่แตะ `client/src/pages/ArcaneFrontier.tsx`, `client/src/components/GameCanvas.tsx`, `client/src/game/scene.ts`, `server/generators/renderDistanceDependencyGraph.ts`, player profiler UI, Workbench/router/map/cache/offline/authority หรือ binary assets | Base `403839b60109719d6216bc35e94724863d5af26f`; branch `ai1/m02-runtime-visibility-coverage`; implementation `36afb87d631d1aadbc5e107afb96827fab474450`; PR [#42](https://github.com/apirak272543-ship-it/A_Survival/pull/42); `git diff --check`, `pnpm check`, focused `6 files/28 tests`, full `118 files/488 tests` และ heap-limited `pnpm build` ผ่าน; AI-0 ต้อง review/merge และตัดสินสถานะบน `main` | ตรวจแบบ pure/read-only ว่า performance budget, block-distance visibility, prefetch, safety padding, mesh telemetry และ broken-object fail-closed policy เชื่อมกันอย่างมีขอบเขต; ไม่อ้าง benchmark, visual/device/mobile/WebView หรือ production acceptance |
