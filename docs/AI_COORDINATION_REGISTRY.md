...
| `NEXT-MOBILE-VIEWPORT-CSS-BOUNDARY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/index.css` | Base `403839b`; exclusive reservation by AI-0; open worker PR audit #9–#28 found no PR claiming `client/src/index.css` | Add a narrow actual safe-area/landscape responsive CSS boundary for the existing player HUD/control anchors and high-risk overflow surfaces. Preserve `mobileViewportPolicy.ts`, `client/index.html`, ArcaneFrontier/GameCanvas behavior and Obsidian-only runtime. Browser widths 320/390/430/768 are viewport evidence only; no real-device/WebView acceptance claim |
