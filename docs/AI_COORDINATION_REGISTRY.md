...
| `NEXT-PERF-CAPABILITY-001` | AI-1 | 🔵 IN_PROGRESS | Runtime capability evidence contract ต่อจาก T-01; reserved files: `server/generators/runtimePerformanceCapability.ts`, `server/runtimePerformanceCapability.test.ts`; ไม่แตะ device benchmark, player UI, Workbench, router authorization, map/cache/offline/authority หรือไฟล์ใน reservation ของ AI-0 | Base `2ca0e1a`; branch `ai1/runtime-performance-capability`; claim ส่งแล้ว | ทำหนึ่ง bounded pure/read-only capability checkpoint, ตรวจ missing device/LOD/culling/pooling evidence เป็น blocker และส่งหลักฐานให้ AI-0 |
