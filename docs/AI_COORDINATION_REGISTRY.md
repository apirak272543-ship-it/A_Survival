...
| `NEXT-ITEM-DETAIL-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/itemDetailSystem.ts`, `server/itemDetailSystem.test.ts` | Base `845a7a9`; exclusive reservation by AI-0 | เพิ่ม pure category-specific item detail facts จาก canonical `ItemDefinition`/`ItemInstance`; ค่า combat ที่ไม่มี field จริงต้องแสดงเป็น unavailable/blocker ไม่สร้าง damage ปลอม; ไม่แก้ ArcaneFrontier UI, persistence caller หรือ runtime mutation |
