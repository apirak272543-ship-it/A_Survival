...
| `NEXT-B02-PHYSICS-CONTRACT-TEST-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/blockPhysicsSystem.ts`, `server/blockPhysicsSystem.test.ts` | Base `d5501db`; exclusive reservation by AI-0. Open worker PR #22 claims `server/blockHazardPlacementCoverageContract.ts` and its test only, not these exact paths | Add focused regression coverage for canonical Obsidian `slab` partial blocking, `thin` non-solid pass-through, cactus hazard contact and cooldown. Keep `blockPhysicsSystem.ts` behavior bounded and source-derived; no fabricated hazard, no future-map expansion, no gravity solver or authoritative persistence claim |## Autonomous worker instructions
