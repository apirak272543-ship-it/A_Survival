...
| `AI1-G05-001` | AI-1 | 🔵 IN_PROGRESS | G-05 runtime asset provenance audit; reserved files: `server/generators/runtimeAssetProvenanceAudit.ts`, `server/runtimeAssetProvenanceAudit.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ตรวจเฉพาะ manifest/credits/source/license/reference-only boundary; ห้ามสร้าง PNG/GLB, แก้ `assetProvenance.ts`, Workbench/router, map/cache/offline/authority หรือไฟล์ใน reservation ของ AI-0/AI-2 | Base `330593c`; branch `ai1/g05-runtime-asset-provenance`; claim ส่งแล้ว | ทำหนึ่ง pure/read-only bounded audit จาก active manifest และ asset credits แล้วส่ง evidence ให้ AI-0 |
