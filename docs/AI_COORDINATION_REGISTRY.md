...
| `AI1-S04-001` | AI-1 | ⚪ WAITING_EVIDENCE | S-04 capability-policy adapter; files: `server/generators/runtimeCapabilityPolicy.ts`, `server/runtimeCapabilityPolicy.test.ts`; รับ caller-provided WebGL/device signals แบบ bounded/fail-safeเท่านั้น; ห้าม mutate tier อัตโนมัติ, อ้าง benchmark, แก้ player UI, Workbench/router, map/cache/offline/authority หรือไฟล์ใน reservation ของ AI-0/AI-2 | Base `04f2cffc`; branch `ai1/s04-capability-policy`; implementation `abcc2e63ce28d85edcbac61f58ceafb44f1f65e1`; PR [#9](https://github.com/apirak272543-ship-it/A_Survival/pull/9); focused 1 file/4 tests, full 109 files/432 tests, `pnpm check`, `git diff --check`, production build ผ่าน | รอ AI-0 ตรวจ diff และหลักฐาน แล้วเปลี่ยนเป็น 🟢 DONE หรือแจ้ง blocker |
