...
| `NEXT-CREDITS-PROVENANCE-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/creditsPresentationContract.ts`, `server/creditsPresentationContract.test.ts` | Base `c5eaeb9`; exclusive reservation by AI-0 | เพิ่ม pure credits sections จาก `ASSET_CREDITS` แยก project-original/license-verified ออกจาก reference-only; ต้องคง source URL/label, attribution และ no runtime asset distribution/publish side effect; ไม่แก้ CreditsSheet/player UI ในรอบนี้ |
