...
| `NEXT-PLANT-ECOLOGY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/plantEcologyPolicy.ts`, `server/plantEcologyPolicy.test.ts` | Base `7e3a3dd`; exclusive reservation by AI-0 | เพิ่ม pure data-driven policy สำหรับ soil/biome/lifecycle และ bounded fictional nutrient/pest/season factorsจาก canonical `PLANT_CATALOG`; ต้องไม่อ้างการจำลอง ecology จริง, ไม่เพิ่ม gameplay mutation, ไม่แก้ `worldFarmSystem.ts` หรือ player UI |
