...
| `AI1-F03-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/plantGrowthStageContract.ts`, `server/plantGrowthStageContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `client/src/game/systems/worldFarmingSystem.ts`, provenance PR ของ AI-2, Workbench/router/map/cache/offline/authority | Base `97e58a65f2af5ce356cfabd80bffd56b21b6f955`; branch `ai1/f03-growth-stage-contract`; implementation/PR pending | pure stage timeline/rehydration projection จาก caller-provided plant state; mature-only eligibility เป็น boolean read-only เท่านั้น; no reward grant, harvest mutation, growth simulation, storage write หรือ player UI |
