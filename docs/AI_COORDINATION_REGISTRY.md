...
| `NEXT-S03-TARGET-FPS-DISCLOSURE-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/pages/ArcaneFrontier.tsx` | Base `3f00602`; exclusive reservation by AI-0; open PR #27 was reviewed as pure server audit and does not claim this file | ปรับข้อความ option 120 FPS และ note ใน settings ให้เป็น high-refresh advisory/non-guarantee อย่างชัดเจน; ไม่แก้ normalization/runtime loop, ไม่วัด FPS จริง, ไม่อ้าง device acceptance และไม่เปลี่ยน map/cache policy |
| `AI1-V01-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/sceneReadabilityCoverageContract.ts`, `server/sceneReadabilityCoverageContract.test.ts`, `docs/SCENE_READABILITY_COVERAGE.md`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `client/src/game/scene.ts`, biome/block/asset manifest owners, GameCanvas/player controls, Workbench/router/map/cache/offline/authority | Base `7dd5c460a5b7bdb54447a58a5fca5d0ada1b3bd7`; branch `ai1/v01-scene-readability-coverage`; implementation `9042357f3f58e295e0edf8ecc75d0e17e579ce91`; PR [#36](https://github.com/apirak272543-ship-it/A_Survival/pull/36); `git diff --check`, `pnpm check`, focused `1 file/4 tests` plus related `3 files/18 tests`, full `118 files/487 tests`, heap-limited `pnpm build` ผ่าน; AI-0 ต้อง review/merge และตัดสินสถานะบน `main` | pure static projection ของ terrain/decor categories, asset IDs, relief/readability metadata และ emissive advisory; no visual screenshot, rendering change, device or production acceptance; build warnings เป็น baseline (`VITE_ANALYTICS_*`, umami `type=module`, Babylon chunk, ignored pnpm fields) และไม่ได้อ้างว่าแก้แล้ว |
