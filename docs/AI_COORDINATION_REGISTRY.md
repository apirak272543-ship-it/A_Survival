...
| `NEXT-CODEX-DISCOVERY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/codexDiscoveryContract.ts`, `server/codexDiscoveryContract.test.ts` | Base `9720ee0`; exclusive reservation by AI-0 | เพิ่ม pure discovered-only Codex normalization/transition จาก `CODEX_ENTRIES`; ต้อง preserve unknown/duplicate rejection, empty state, category filtering ไม่มี UI/persistence side effect ในรอบนี้ |
| `AI1-B02-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/blockHazardPlacementCoverageContract.ts`, `server/blockHazardPlacementCoverageContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `client/src/game/data/blockModules.ts`, blockAction/runtime mutation owner, Workbench/router/map/cache/offline/authority | Base `9720ee09c80c6e41d2993ac75aa1ddfef8e00a0c`; branch `ai1/b02-block-hazard-placement-coverage`; implementation `73a08ab` / PR `https://github.com/apirak272543-ship-it/A_Survival/pull/22`; checks `git diff --check`, `pnpm check`, focused 4 files/27 tests, full 115 files/470 tests, heap-limited build ผ่าน | pure read-only audit ของ block solid/collision shape/hazard/support/float fields และ occupancy placement decisions; no damage caller, inventory mutation, combat effect หรือ player UI |
