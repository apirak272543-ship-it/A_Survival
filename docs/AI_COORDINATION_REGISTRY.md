...
| `AI1-F04-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/plantEffectCoverageContract.ts`, `server/plantEffectCoverageContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `plantCatalog.ts`, `worldFarmSystem.ts`, player UI, router/Workbench/map/cache/offline/authority | Base `1085ac2d4c8c5297bada95f84b270e8e37e17761`; branch `ai1/f04-plant-effect-coverage`; implementation/PR pending | pure read-only audit ของ PlantEffect kind/power/radius, caps และ cactus/repellent boundary จาก canonical catalog; no medical claim, damage/repel caller, gameplay mutation หรือ player acceptance |
