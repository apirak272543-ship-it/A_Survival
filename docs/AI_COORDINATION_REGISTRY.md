...
| `NEXT-PLANT-ECOLOGY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/plantEcologyPolicy.ts`, `server/plantEcologyPolicy.test.ts` | Base `7e3a3dd`; exclusive reservation by AI-0 | เพิ่ม pure data-driven policy จาก `PLANT_CATALOG` สำหรับ soil/biome/lifecycle และ bounded fictional nutrient/pest/season factors; ไม่อ้าง ecology จริง, ไม่แก้ `worldFarmSystem.ts`, ไม่เพิ่ม gameplay mutation หรือ player UI |
| `AI1-C01-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/codexDiscoveryCoverageContract.ts`, `server/codexDiscoveryCoverageContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `client/src/game/systems/codexSystem.ts`, player UI/Vault, `itemDetailSystem.ts`, Workbench/router/map/cache/offline/authority | Base `5107b553142866471e10a849a0da7545d5428c39`; branch `ai1/c01-codex-discovery-coverage`; implementation `1cb1d55` / PR `https://github.com/apirak272543-ship-it/A_Survival/pull/20`; checks `git diff --check`, `pnpm check`, focused 3 files/10 tests, full 113 files/459 tests, heap-limited build ผ่าน | pure read-only audit ของ Codex categories, subcategories และ discovered-only filtering จาก canonical entries; no discovery persistence, UI integration, item fact fabrication หรือ player mutation |
