...
| `NEXT-QUEST-REWARD-PERSISTENCE-INTEGRATION-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/storage/indexedDb.ts`, `client/src/game/systems/questRewardPendingAction.ts`, `server/syncActionValidation.ts`, `server/db.ts`, `server/questRewardPersistenceIntegration.test.ts` | Base `04f2cff`; exclusive reservation by AI-0; claim is for queue validation and server sync allow-list only | เพิ่ม fail-closed serializer gate สำหรับชนิด `quest-reward-dispatch` และให้ `writeGameSyncBatch` รับเฉพาะ payload ที่ผ่าน `isSafeQuestRewardDispatchPayload`; ห้ามเพิ่ม gameplay caller, reward grant, quest completion, ability/reputation unlock, session UI หรือ future-map write |
...
| `AI1-S04-001` | AI-1 | 🔵 IN_PROGRESS | S-04 capability-policy adapter; reserved files: `server/generators/runtimeCapabilityPolicy.ts`, `server/runtimeCapabilityPolicy.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; รับ caller-provided WebGL/device signals แบบ bounded/fail-safeเท่านั้น; ห้าม mutate tier อัตโนมัติ, อ้าง benchmark, แก้ player UI, Workbench/router, map/cache/offline/authority หรือไฟล์ใน reservation ของ AI-0/AI-2 | Base `04f2cffc`; branch `ai1/s04-capability-policy`; claim ส่งแล้ว | ทำ pure/read-only policy checkpoint ตรวจ recommendation, fallback, profile ceiling และ explicit missing-evidence blockers |
