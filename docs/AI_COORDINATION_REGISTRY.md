...
| `AI1-S02-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/settingsScopeCoverageContract.ts`, `server/settingsScopeCoverageContract.test.ts`, `docs/SETTINGS_SCOPE_COVERAGE.md`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `cameraModes.ts`, `offlineMapState.ts`, localStorage callers, ArcaneFrontier, Workbench/router/map/cache/offline/authority | Base `bb51a5572702e023854b1d3d70c5f730a559048c`; branch `ai1/s02-settings-scope-coverage`; implementation `9c6facc` / PR `https://github.com/apirak272543-ship-it/A_Survival/pull/34`; checks `git diff --check`, `pnpm check`, focused 4 files/16 tests, full 118 files/487 tests, heap-limited build ผ่าน | pure scope projection แยก global/in-map settings, canonical in-map defaults/allow-list และ missing persistence/UI owners; no settings write, route/UI integration, device acceptance หรือ future-map mutation |
