...
| `NEXT-CREDITS-UI-BOUNDARY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/pages/ArcaneFrontier.tsx` | Base `388330d`; exclusive reservation by AI-0. Worker PR queue was audited; no open PR currently claims this file | เชื่อม `CreditsSheet` กับ `createCreditsPresentation(ASSET_CREDITS)` ให้ใช้ canonical sections/entries แบบ fail-closed โดยคง reference-only และ needs-review labels; ไม่เพิ่ม asset bytes, publish, persistence, network หรือ creator controls |
