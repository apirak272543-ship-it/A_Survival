...
| `NEXT-ITEM-DETAIL-UI-BOUNDARY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/pages/ArcaneFrontier.tsx` | Base `a086d57`; exclusive reservation by AI-0 | เชื่อม `ItemDetailSheet` (long-press) กับ `getItemCategoryDetail` เพื่อแสดง category-specific facts (usage, stack, plant soil/effect, block placement, tool tag) จาก canonical definitions; คง attack damage เป็น unavailable เมื่อไม่มี owner และไม่สร้าง stat ปลอม |
| `AI1-S01-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/cameraModeCoverageContract.ts`, `server/cameraModeCoverageContract.test.ts`, `docs/CAMERA_MODE_COVERAGE.md`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `cameraModes.ts`, GameCanvas, camera bridge, player UI/persistence, router/Workbench/map/cache/offline/authority | Base `33778bd5e643efad402e47c60880b01c0a0718b3`; branch `ai1/s01-camera-mode-coverage`; implementation `69b9ca0` / PR `https://github.com/apirak272543-ship-it/A_Survival/pull/32`; checks `git diff --check`, `pnpm check`, focused 4 files/18 tests, full 118 files/487 tests, heap-limited build ผ่าน | pure coverage projection ของ 3 canonical in-map camera modes, poses, labels และ fallback; no touch/collision/device-size acceptance, camera caller integration หรือ player state write |
