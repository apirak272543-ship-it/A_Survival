...
| `AI1-M02-001` | AI-1 | 🔵 IN_PROGRESS | M-02 render/load ใกล้ player และ visibility/streaming policy; reserved files: `server/generators/runtimeRenderLoadVisibility.ts`, `server/runtimeRenderLoadVisibility.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ห้ามเปิด GPU/mobile claim และห้ามแตะ player UI, Workbench, router authorization, map/cache/offline/authority หรือไฟล์ใน reservation ของ AI-0 | Base `959d3d3`; branch `ai1/m02-render-load-visibility`; claim ส่งแล้ว | ทำหนึ่ง bounded pure/read-only checkpoint, ตรวจ render/load visibility policy จาก owner จริง และส่ง evidence ให้ AI-0 |
