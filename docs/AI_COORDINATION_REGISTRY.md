...
| `AI1-F01-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/plantCatalogCoverageContract.ts`, `server/plantCatalogCoverageContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `server/generators/plantAssetProvenanceDependencyGraph.ts` หรือไฟล์ provenance ใน PR ของ AI-2, ไม่แก้ Workbench/router/map/cache/offline/authority | Base `8a2bf5e2ca4a1dc8ef9eaa3162ce43db0d2dc131`; branch `ai1/f01-plant-catalog-coverage`; implementation/PR pending | pure read-only coverage audit สำหรับ canonical 300-plant catalog: deterministic biome/soil/family/effect/asset coverage, no binary generation, no registry/cache write, no player runtime mutation |
