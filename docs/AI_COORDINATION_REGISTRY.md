...
| `AI1-T01-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/generators/runtimePerformanceCaptureContract.ts`, `server/runtimePerformanceCaptureContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ห้ามแก้ `creatorRouter.ts`, `CreatorDomainWorkbench.tsx`, player profiler UI, DB/cache/storage หรือ runtime authority | Base `937441ada1da61799b303c9b3c0ae8a0e4e8ce24`; branch `ai1/t01-performance-capture-v2`; implementation `025919e` / PR `https://github.com/apirak272543-ship-it/A_Survival/pull/12`; checks `git diff --check`, `pnpm check`, focused 3 files/14 tests, full 111 files/444 tests, heap-limited build ผ่าน | pure bounded telemetry snapshot → profiler projection → export-only metadata; no controlled device capture, durable history, registry/cache write, asset generation, adaptive tier mutation หรือ player visibility |
| `NEXT-ASSET-PROVENANCE-001` | AI-0 / Main Integrator | 🟡 RESERVED | `server/generators/assetProvenanceDependencyGraph.ts`, `server/assetProvenanceDependencyGraph.test.ts` | Base `4ac30ff`; exclusive reservation by AI-0 | สร้าง pure graph adapter เชื่อม item/plant asset IDs กับ active manifest และ credit/provenance records; missing/mismatch จะเป็น blockers; ห้ามสร้าง PNG/GLB, แก้ Workbench/router หรือ import/publish runtime |
