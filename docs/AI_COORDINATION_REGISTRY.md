...
| `AI1-G06-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/aiNpcPolicyContract.ts`, `server/aiNpcPolicyContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `server/aiNpcService.ts`, `server/routers.ts`, provider/network/DB/UI | Base `c5eaeb95d7d3ac0f390040afb56a54b12d45ef08`; branch `ai1/g06-ai-npc-policy-contract`; implementation/PR pending | pure read-only validation/policy projection สำหรับ one optional AI NPC: default disabled, Obsidian-only map, max-one allow-listed action, bounded request/output metadata; no provider call, secret, background loop, persistence หรือ gameplay mutation |
