...
| `AI1-F01-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/plantCatalogCoverageContract.ts`, `server/plantCatalogCoverageContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ `server/generators/plantAssetProvenanceDependencyGraph.ts` หรือไฟล์ provenance ใน PR ของ AI-2, ไม่แก้ Workbench/router/map/cache/offline/authority | Base `8a2bf5e2ca4a1dc8ef9eaa3162ce43db0d2dc131`; branch `ai1/f01-plant-catalog-coverage`; implementation `4a7cc0b` / PR `https://github.com/apirak272543-ship-it/A_Survival/pull/14`; checks `git diff --check`, `pnpm check`, focused 3 files/9 tests, full 111 files/443 tests, heap-limited build ผ่าน | pure read-only coverage audit สำหรับ canonical 300-plant catalog: deterministic biome/soil/family/effect/asset coverage; no binary generation, registry/cache write, runtime import, player mutation or manifest binding |
| `NEXT-ANIMATION-MOTION-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/animationMotionPolicy.ts`, `server/animationMotionPolicy.test.ts` | Base `ff29617`; exclusive reservation by AI-0 | เพิ่ม pure distance/visibility/reduced-motion animation policy และ bounded motion profile metadata; ไม่สร้าง binary asset, ไม่แก้ GameCanvas caller, ไม่อ้าง skeleton/retarget/device benchmark และไม่เปิด player UI |
