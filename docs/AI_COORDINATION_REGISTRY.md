...
| `NEXT-PERF-CAPABILITY-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/runtimePerformanceCapability.ts`, `server/runtimePerformanceCapability.test.ts` | Base `5dc54e1`; exclusive reservation by AI-0 | เพิ่ม pure one-time capability normalization/advisory contract สำหรับ WebGL/WebGPU/CPU/RAM/touch/storage inputs; ไม่ผูกเข้ากับ render loop, ไม่เปลี่ยน tier อัตโนมัติ และไม่อ้าง device benchmark |
| `AI1-T01-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/generators/runtimePerformanceCaptureContract.ts`, `server/runtimePerformanceCaptureContract.test.ts`, `docs/AI_COORDINATION_REGISTRY.md`; ห้ามแก้ `creatorRouter.ts`, `CreatorDomainWorkbench.tsx`, player profiler UI, DB/cache/storage หรือ runtime authority | Base `937441ada1da61799b303c9b3c0ae8a0e4e8ce24`; branch `ai1/t01-performance-capture-v2`; implementation/PR pending | pure bounded telemetry snapshot → profiler projection → export-only metadata; claim exact files only; no controlled device capture, durable history, registry/cache write, asset generation, adaptive tier mutation หรือ player visibility |
