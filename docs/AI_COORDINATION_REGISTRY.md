...
| `NEXT-MOBILE-VIEWPORT-001` | AI-0 / Main Integrator | 🟡 RESERVED | `client/src/game/systems/mobileViewportPolicy.ts`, `server/mobileViewportPolicy.test.ts` | Base `c4d2b1e`; exclusive reservation by AI-0 | เพิ่ม pure one-time normalization ของ viewport/orientation/safe-area/touch/fullscreen signals และ layout advisory สำหรับ landscape WebView; ไม่แก้ `index.css`, `client/index.html`, GameCanvas หรืออ้าง real-device acceptance ในรอบนี้ |
| `AI1-O04-001` | AI-1 ตาม autonomous worker | 🔵 IN_PROGRESS | `server/generatorToolDependencyPolicy.ts`, `server/generatorToolDependencyPolicy.test.ts`, `docs/GENERATOR_TOOL_INVENTORY.md`, `docs/AI_COORDINATION_REGISTRY.md`; ไม่แตะ generator implementations, Workbench/router, map/cache/offline/authority | Base `d5e0270b92fd72967ebda12e3e236eb8e4c1652e`; branch `ai1/o04-generator-tool-dependency-policy`; implementation/PR pending | pure inventory/dependency gate ของเครื่องมือ generator ที่ตรวจได้ใน repository; no external execution, secret, background process, live DB migration, player generator UI หรือ fabricated tool success |
