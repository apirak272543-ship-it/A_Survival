...
| T-04 | Common Generator API / Game Creation Engine ครอบคลุม world/quest/structure/dungeon/crafting/economy/audio/weather/vegetation/farming/test/profiling/migration | มี `server/generators/commonGeneratorApi.ts` เป็น foundation แบบ backend/data-only พร้อม artifact schema, registry, generate/validate/preview/save/export/version; generator รายด้านและ no-code engine ยังไม่มี | PARTIAL | เชื่อม plugin/tool รายด้านทีละรายการ, เพิ่ม durable content registry/orchestrator และคง no player generator UI |
