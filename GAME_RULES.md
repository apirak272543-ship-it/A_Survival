...
| Game Images | ทุกภาพเริ่มจาก Gemini brief ในรูป “สร้างพร้อมสำหรับ… / จุดประสงค์ของภาพ…” แล้วดึงภาพสุดท้ายจาก Pollinations.ai Image API ด้วย prompt ภาษาอังกฤษ, `width`, `height`, `model`, `nologo=true` และ `seed` ที่บันทึกได้ |

## 2. แก่นของเกมและแพลตฟอร์ม

เกมเป็น open-world survival ที่ผสานแฟนตาซี เวทมนตร์ สิ่งลี้ลับ ซอมบี้ และวิทยาศาสตร์/เทคโนโลยีต่างดาว โดยมีเป้าหมายเป็นเว็บเกม mobile-first สำหรับมือถือแนวนอน ซึ่งในอนาคตต้องพร้อมห่อ URL เป็น APK เกมต้องมีความรู้สึกเป็นโลกที่กว้างและอิสระ แต่แต่ละแผนที่ควบคุมขอบเขตไว้ประมาณ 1–1.5 กิโลเมตรจากจุดเกิดเพื่อคงประสิทธิภาพของมือถือ
...
เมื่อภาพพร้อม ต้องใช้เป็น texture, background, sprite หรือ `<img>` ของเกมแทนการวาดตัวแทนแบบเรขาคณิตบน Canvas แอสเซ็ตที่ดาวน์โหลดต้องเข้าคิวแคชสำหรับการเล่นออฟไลน์ และต้องเก็บ prompt/seed/model ไว้กับ metadata เพื่อสร้างซ้ำได้อย่างสอดคล้อง
...
ระบบต้องรองรับอย่างน้อย **100 map modules** ที่แยกส่งมอบและ commit ได้ทีละแผนที่ แต่ละแผนที่มี biome ที่สอดคล้องทั้งฉาก NPC, landmark, ทรัพยากร, regular monster, elite, surprise encounter และ event boss เฉพาะถิ่น ชุด biome เริ่มต้นครอบคลุม Obsidian Frontier, Ashen Hellscape, Mars Expanse, Saharan Glass, Congo Verdant, Stonecrest Range, Wildpine Highlands และ Astral Drift โดยต้องขยายเป็นครอบครัวของแผนที่ที่หลากหลาย
